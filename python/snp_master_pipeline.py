	...
	with open('t_filter.rdf.csub.vcf') as fh:
		for line in fh:
			if '#' not in line[0]:
				ll=line.split('\t')
				ll=ll[7].split('|')	
				if ('missense_variant' or 'disruptive_inframe_deletion' or 'frameshift_variant' or 'stop_gained' or 'disruptive_inframe_insertion' or 'exon_loss_variant' or 'disruptive_inframe_deletion' or 'start_loss' or 'stop_loss') in ll:
					out+=line
...
def snp_tidy():
	...
	os.rename('t_filter.rdf.csub.selectann.720sub.vcf','final_results/t_filter.rdf.csub.selectann.720sub.vcf')
