	...
	with open('t_filter.rdf.csub.vcf') as fh:
		for line in fh:
			if '#' not in line[0]:
				ll=line.split('\t')
				ll=ll[7].split('|')	
				if ("HIGH" or "MODERATE" or "missense_variant") in ll:
					out+=line
			...
			for line in fh:
				if '#' not in line[0]:
					...
					if linecounter % 100000 == 0:
						...
						minutes=timeelapsed / 60
						displayminutes=format(minutes, '.2f') 
						sys.stdout.write(str(linecounter)+' total variants processed, last 100k in '+str(displayminutes)+' minutes\n')
						sys.stdout.flush()
						oldtime=newtime
					ll = line.split('\t')
					chroosome=ll[0]
